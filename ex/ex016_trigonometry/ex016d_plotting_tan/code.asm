...
tangent_function: ; takes 1 double from [rsp+8] and computes tangent thereof
...
START:
	...
	mov [.x_param+8],r12
	mov [.tangent_data+16],r12
	mov [.tangent_param+8],r13
	mov [.tangent_data+26],r13	
	...
	; evaluate function of x_param into tangent_param
	mov rdi,.tangent_param
	mov rsi,.x_param
	mov rdx,tangent_function
...
.plot_structure:
	...
	dq .tangent_data; address of linked list for datasets {*+24}
	dw 480; plot width (px) {*+32}
	dw 200; plot height (px) {*+34}
	dw 5; plot margins (px) {*+36}
	dq -8.0; x-min (double) {*+38}
	dq 8.0; x-max (double) {*+46}
	dq -20.0; y-min (double) {*+54}
	dq 20.0; y-max (double) {*+62}
	...
	db 5; number of major x-ticks {*+88}
...
.tangent_data:
	...
	dq .tangent_label; address of null-terminated label string {*+8}
