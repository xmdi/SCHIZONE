...
START:
	...
	call tick_time; save the initial timestamp time value
...
.loop:	; the loop we are timing
	...
	call tock_time	; compute the microseconds elapsed since "tick_time"
	mov rbx,rax	; save microseconds elapsed in {rbx}
	
	call tock_cycles; compute the cycles elapsed since "tick_cycles"
			; save cycles elapsed in {rax}

	xor rdx,rdx
	div rbx		; {rax} contains cycles/microsecond
	mov rdx,1000000
	imul rax,rdx	; adjust {rax} to be in cycles/second (Hz)	
...
.grammar:
	db ` loop iterations completed at Hz.\n`
